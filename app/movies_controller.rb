...
def can_be_created_with_a_hash_of_attributes
  ...
  movie = Movie.create(attributes) # create not new because that's the initializing method
#   binding.pry
end
...
def can_find_the_first_item_from_the_database_using_id
  Movie.all.select{|inst| inst.id == 1}.first
end
...
#   binding.pry
#   a = Movie.select(:id).where('title == That One Where the Guy Kicks Another Guy Once')
...
def can_destroy_all_items_at_once
  ...
  Movie.all.each{|inst| inst.destroy}
end